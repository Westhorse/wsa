<div style="width:100%;font-family:lato,'helvetica neue',helvetica,arial,sans-serif;padding:0;Margin:0">
    <div style="background-color:#151719">
        @php
        $template = DB::table('email_templates')->where('slug', 'one_to_one_sender_request_email_template')->value('body');
        $faker = Faker\Factory::create();
        $fakeEmail = $faker->email;
        $fakeName = $faker->name;
        $fakeNameSecound = $faker->name;
        $receiver_company_name = $faker->name;
        $receiver_company_city = $faker->name;
        $receiver_company_country = $faker->name;
        ...
        $template = str_replace('{{sender_email}}', $fakeEmail, $template);
        $template = str_replace('{{receiver_name}}', $fakeName, $template);
        $template = str_replace('{{sender_name}}', $fakeNameSecound, $template);
        $template = str_replace('{{receiver_job_title}}', 'CIO', $template);
        $template = str_replace('{{receiver_title}}', 'Mr', $template);
        $template = str_replace('{{sender_title}}', 'Mr', $template);
        $template = str_replace('{{time_from}}', '05:00', $template);
        $template = str_replace('{{time_to}}', '05:30', $template);
        $template = str_replace('{{day_date}}', '5-10-2024', $template);
        $template = str_replace('{{receiver_company_name}}', $receiver_company_name, $template);
        $template = str_replace('{{receiver_company_city}}', $receiver_company_city, $template);
        $template = str_replace('{{receiver_company_country}}', $receiver_company_country, $template);
        $template = str_replace('{{login_button}}', $login_button, $template);
       @endphp
