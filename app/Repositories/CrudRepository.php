...
class CrudRepository implements ICrudRepository
{
    ...
    public function allEvent($with = [], $conditions = [], $columns = array('*'), $useNetworkId = false, $useCollection = false)
    {
    ...
    public function allOrder($with = [], $conditions = [], $columns = array('*'), $useNetworkId = false, $useCollection = false)
    {
        $conferenceId = request()->header('X-Conference-Id');
        $currentConferenceCollect = Conference::where('id', $conferenceId)->first();
        $order_by = request(Constants::ORDER_BY) ?? "id";
        ...
        // إضافة شرط البحث بواسطة بريد المستخدم
        if (!empty($conditions['user_email'])) {
            $query->whereHas('user', function ($q) use ($conditions) {
                $q->where('email', 'info@pyramidsfreight.com');
            });
        }
        if (!empty($relationFilter['orderStatus'])) {
            $query->whereIn('status', $relationFilter['orderStatus']);
        }
        $all_conditions = array_merge($conditions, $filters);
    ...
    public function all($with = [], $conditions = [], $columns = ['*'], $useNetworkId = false, $useCollection = false)
    {
        $networkId = request()->header('X-Network-ID');
        $currentNetworkCollect = Network::where('id', $networkId)->value('collection');
        $order_by = request(Constants::ORDER_BY) ?? "id";
